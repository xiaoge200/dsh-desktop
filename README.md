...
2. 选择最新版本，下载对应系统的安装包：
   ...
   - **macOS**：`.dmg`（Apple Silicon / arm64；要求 macOS 11 及以上。Intel 版暂不提供）
   - **Linux**：暂不提供（打包链路待修复）
...
**Q：网页里的「插件市场」提示要配置 pnpm（找不到 pnpm / 没有权限 / 可执行文件已存在）？**
0.1.8 起桌面端已自带 pnpm，插件市场直接用内置的，不需要系统里装。若你在 **0.1.7 及更早版本**
遇到这条提示，按提示在终端执行一次即可（装好后重启应用）：

```bash
npm i -g pnpm --force        # Windows 需要用管理员身份运行终端
brew install pnpm            # macOS（Homebrew）
```

装不上也没关系——设置窗口「插件」页里的市场走内置 npm，不受影响。
